#!/usr/bin/env python
"""Voice Activity Detection and Speech-to-Text with Memory Processing.

This module provides a VAD transcriber that uses in-memory audio processing
based on the approach from processor.py that works reliably without temporary files.
"""
...
if __name__ == '__main__':
    # When run directly as a script, import from the current directory
...
class MemoryVADTranscriber(QObject):
    ...
    def _process_audio_chunk(self, audio_chunk):
        """Process a single chunk of audio data using our SpeechToText class.
        ...
        try:
            ...
            # Normalize audio (ensure values between -1.0 and 1.0)
            max_val = np.max(np.abs(audio_data))
            if max_val > 0:
                audio_data = audio_data / max_val * 0.9  # Scale to 90% to prevent clipping
            
            # Use the SpeechToText class from stt.py since that's known to work
            print(f"Calling SpeechToText with audio shape={audio_data.shape}")
            
            # Try multiple approaches in sequence
            print("Trying direct transcription approach...")
            result = self.stt.transcribe(audio_data, self.sample_rate)
            
            if result and result.strip():
                print(f"Direct transcription successful: {repr(result)}")
                return self._clean_transcript(result)
            
            # If direct approach fails, try with the WAV file method from testvad.py
            print("Direct method failed, trying WAV file approach...")
            ...
            try:
                ...
                print(f"Created temp WAV file: {temp_filename}")
                
                # Read back the WAV file
                with wave.open(temp_filename, 'rb') as wf:
                    sample_rate = wf.getframerate()
                    raw_data = wf.readframes(wf.getnframes())
                    wav_audio = np.frombuffer(raw_data, dtype=np.int16).astype(np.float32) / 32768.0
                
                # Try transcription with the WAV file data
                print(f"Transcribing WAV file audio: length={len(wav_audio)}")
                result = self.stt.transcribe(wav_audio, sample_rate)
                ...
                if result and result.strip():
                    print(f"WAV file transcription successful: {repr(result)}")
                    return self._clean_transcript(result)
                    
                print("WAV file transcription failed")
                
            except Exception as wav_error:
                print(f"WAV file approach error: {wav_error}")
                import traceback
                traceback.print_exc()
                try:
                    import os
    ...
    def _try_standard_processing(self, audio_chunk):
        """Standard processing approach from processor.py"""
        try:
            # 1. Convert to numpy array if needed
            if not isinstance(audio_chunk, np.ndarray):
                audio_data = np.frombuffer(audio_chunk, dtype=np.float32)
            else:
                audio_data = audio_chunk.copy()  # Create a copy to avoid modifying original
                
            # 2. Ensure normalization (-1.0 to 1.0)
            max_val = np.max(np.abs(audio_data))
            if max_val > 0:
                audio_data = audio_data / max_val * 0.9  # Scale to 90% to prevent potential clipping
            
            # 3. Reshape for STT model - must be [1, samples] for MoonshineSTT
            if len(audio_data.shape) == 1:
                audio_data = audio_data.reshape(1, -1)
                
            # 4. Ensure float32 format
            if audio_data.dtype != np.float32:
                audio_data = audio_data.astype(np.float32)
                
            # Display diagnostic info
            print(f"Standard processing: shape={audio_data.shape}, dtype={audio_data.dtype}")
            print(f"Audio stats: min={np.min(audio_data):.3f}, max={np.max(audio_data):.3f}, mean={np.mean(audio_data):.3f}")
            
            # 5. Direct call to STT model
            print("Calling STT model with standard processing...")
            text = self.stt_model.stt((self.sample_rate, audio_data))
            print(f"STT result (standard): {repr(text)}")
    ...
    def _try_with_padding(self, audio_chunk):
        """Try processing with silence padding before and after the speech"""
        try:
            # 1. Convert to numpy array if needed
            if not isinstance(audio_chunk, np.ndarray):
                audio_data = np.frombuffer(audio_chunk, dtype=np.float32)
            ...
            # 2. Add silence padding (0.5 seconds) at beginning and end
            padding = np.zeros(int(self.sample_rate * 0.5), dtype=np.float32)
            padded_audio = np.concatenate([padding, audio_data, padding])
            
            # 3. Apply stronger normalization (some models need this)
            max_val = np.max(np.abs(padded_audio))
            if max_val > 0:
                padded_audio = padded_audio / max_val  # Use full scale normalization
            
            # 4. Reshape for STT model
            if len(padded_audio.shape) == 1:
                padded_audio = padded_audio.reshape(1, -1)
            
            print(f"Padded processing: shape={padded_audio.shape}")
            print(f"Padded audio stats: min={np.min(padded_audio):.3f}, max={np.max(padded_audio):.3f}")
            
            # 5. Call STT model
            print("Calling STT model with padded audio...")
            text = self.stt_model.stt((self.sample_rate, padded_audio))
            print(f"STT result (padded): {repr(text)}")
            ...
            print(f"Padded processing error: {e}")
            return ""
            
    def _try_with_resampling(self, audio_chunk):
        """Try resampling the audio to a different rate before processing"""
        try:
            import librosa
            ...
            # 2. Reshape to 1D for resampling if needed
            if len(audio_data.shape) > 1:
                audio_data = audio_data.flatten()
            
            # 3. Resample to a slightly different rate
            # Sometimes the exact sample rate matters to speech recognition
            target_sr = 22050  # Common alternative sample rate
            resampled = librosa.resample(audio_data, orig_sr=self.sample_rate, target_sr=target_sr)
            ...
            print(f"Resampled processing: shape={resampled.shape}, rate={target_sr}")
            
            # 6. Call STT model with resampled audio
            print("Calling STT model with resampled audio...")
            text = self.stt_model.stt((target_sr, resampled))
            print(f"STT result (resampled): {repr(text)}")
    ...
    def _try_wav_file_approach(self, audio_chunk):
        """Fall back to the WAV file approach that was known to work"""
        try:
            import wave
            import os
            ...
            try:
                # Normalize audio
                if isinstance(audio_chunk, np.ndarray):
                    normalized_audio = audio_chunk.copy()
                else:
                    normalized_audio = np.frombuffer(audio_chunk, dtype=np.float32)
                    
                max_val = np.max(np.abs(normalized_audio))
                if max_val > 0:
                    normalized_audio = normalized_audio / max_val * 0.9
                ...
                # Call STT model
                print(f"Calling STT model with WAV data: shape={audio_data.shape}, rate={sample_rate}")
                text = self.stt_model.stt((sample_rate, audio_data))
                print(f"STT result (WAV): {repr(text)}")
                ...
                print(f"WAV file approach error: {e}")
            ...
            print(f"WAV file approach setup error: {e}")
            return ""
    ...
    def _clean_transcript(self, text: str) -> str:
        ...
        # Remove repetitive number sequences
        clean_words = []
...
class MemoryVADTranscriberUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Memory VAD & Transcription")
        ...
        self.transcriber = MemoryVADTranscriber(vad_threshold=0.5)
